...
import {IBaseOracle} from "../interfaces/hai/oracles/IBaseOracle.sol";
import {IDelayedOracle} from "../interfaces/hai/oracles/IDelayedOracle.sol";
 ...
 */
contract TestSettableDelayedOracle is IBaseOracle, IDelayedOracle {
    // --- Registry ---
    ...
    constructor(IBaseOracle _priceSource, uint256 _updateDelay) {
        if (address(_priceSource) == address(0))
            revert DelayedOracle_NullPriceSource();
    ...
    function setCurrentFeed(uint256 _value, bool _isValid) external {
        _currentFeed = Feed(_value, _isValid);
        emit UpdateResult(_currentFeed.value, block.timestamp);
    }

    function setNextFeed(uint256 _value, bool _isValid) external {
        _nextFeed = Feed(_value, _isValid);
        emit UpdateResult(_currentFeed.value, block.timestamp);
    }

    function getCurrentFeed() external view returns (Feed memory) {
        return _currentFeed;
    }

    // Add a new function to set _nextFeed
    function getNextFeed() external view returns (Feed memory) {
        return _nextFeed;
    }
    ...
    function _getPriceSourceResult()
        internal
        view
        returns (uint256 _priceFeedValue, bool _hasValidValue)
    {
